...
const posts = [];
page.on("request", (req) => { if (req.method() === "POST" && req.headers()["next-action"]) posts.push(req.headers()["next-action"]); });
await page.goto(BASE + "/", { waitUntil: "domcontentloaded", timeout: 30000 });
await T(4000);
await page.evaluate(() => { const b = [...document.querySelectorAll("a,button")].find((x) => x.textContent.trim() === "Topics"); if (b) b.click(); });
await T(6000);
const cards = await page.evaluate(() => [...document.querySelectorAll("button")].map((el) => el.textContent.replace(/\s+/g, " ").trim()).filter((t) => /Questions Available/.test(t)).slice(0, 3));
console.log("cards:", JSON.stringify(cards, null, 1));
console.log("action posts:", posts.length, posts[0] ? posts[0].slice(0, 16) : "none");
const errs = [];
