...
const username = `dshot_${Date.now().toString(36)}`;
...
const sres = await fetch(`${API}/sessions`, { method: "POST", headers: h, body: JSON.stringify({ user_id: u.id }) });
const sess = await sres.json();
const tres = await fetch(`${API}/sessions/${sess.id}/tokens`, { method: "POST", headers: h, body: JSON.stringify({ expires_in_seconds: 3600 }) });
...
async function openSession(width) {
...
for (const width of WIDTHS) {
  log(`--- ${width} ---`);
  const { ctx, page } = await openSession(width);

  // Quick Practice: Q1 + feedback
  ...
  await T(900);
  await wt(page.screenshot({ path: path.join(OUT, `fid-practice-q1-${width}.png`) }), 15000, "shot pq1");
  await wt(clickOption(page), 8000, "answer");
  await T(1200);
  await wt(page.screenshot({ path: path.join(OUT, `fid-practice-feedback-${width}.png`) }), 15000, "shot pfb");

  // finish all 10 → results
  for (let target = 2; target <= 10; target++) {
    ...
    await wt(clickOption(page), 8000, `ans${target}`);
    await T(500);
  }
  await wt(clickText(page, "Finish Practice", true), 8000, "finish");
  await wt(waitFor(page, async () => /practice completed|exam completed/i.test(await bodyTxt(page).catch(() => ""))), 12000, "presult");
  await T(1200);
  await wt(page.screenshot({ path: path.join(OUT, `fid-practice-results-${width}.png`) }), 15000, "shot pres");

  // Exam: instructions + Q1 (palette visible) + results
  ...
  await T(800);
  await wt(page.screenshot({ path: path.join(OUT, `fid-exam-instructions-${width}.png`) }), 15000, "shot instr");
  await wt(clickUntil(page, "Start Simulation", true, /question 1\s*\//i), 20000, "sim");
  await T(1200);
  await wt(page.screenshot({ path: path.join(OUT, `fid-exam-q1-${width}.png`) }), 15000, "shot eq1");
  ...
  await wt(waitFor(page, async () => /exam completed/i.test(await bodyTxt(page).catch(() => ""))), 12000, "eresult");
  await T(1200);
  await wt(page.screenshot({ path: path.join(OUT, `fid-exam-results-${width}.png`) }), 15000, "shot eres");
  await ctx.close();
}

await browser.close();
log("desktop screenshots captured");
process.exit(0);
