...
await T(4000);
await page.evaluate(() => { const b = [...document.querySelectorAll("a,button")].find((x) => x.textContent.trim() === "Analytics"); if (b) b.click(); });
await T(6000);
const txt = await page.evaluate(() => document.body.innerText.replace(/\s+/g, " ").trim());
const i = txt.indexOf("Overall Accuracy");
console.log("AROUND KPI:", JSON.stringify(txt.slice(Math.max(0, i - 60), i + 200)));
